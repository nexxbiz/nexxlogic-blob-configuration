...
public class BlobContainerClientFactoryTests
{
    ...
    [Fact]
    public void GetBlobContainerClient_When_ContainerUrl_WithNonSasQueryParams_UsesCredentials()
    {
        // Arrange - URL with non-SAS query parameters (should NOT be treated as SAS)
        var blobConfig = new BlobConfigurationOptions
        {
            BlobContainerUrl = "https://mystorageaccount.blob.core.windows.net/configuration?timeout=30&api-version=2021-06-08",
            ContainerName = "configuration"
        };
        ...
        // Assert - Should use authenticated client because query params are NOT SAS tokens
        Assert.Same(mockContainerClient, result);
        blobServiceClientFactory.Received(1).CreateBlobServiceClient(blobConfig);
        mockBlobServiceClient.Received(1).GetBlobContainerClient("configuration");
    }
